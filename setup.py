...
def get_requirements(file_path:str)->List[str]:
    ...
    with open(file_path) as file_obj:
        ...
        requirements = [req.replace("\n","") for req in requirements] # Replacing \n with blank space to read the requirments.txt file
        
        if HYPHEN_E_DOT in requirements:
            requirements.remove(HYPHEN_E_DOT) # Remove -e . because it will get triggered in setup.py and will cause error
    return requirements
setup(
...
packages = find_packages(), # It will consdier src as the packages. Wherever __init__ will be there
